...
* **Method:** `GET`
* **Access:** Private
* **Query Parameters:** `query` (string, required, 1–100 Unicode code points)
* **Success Response:** `200 OK` (Utilizes Redis Cache if available)
* **Error Response:** `400 Bad Request` when the query is missing, empty, or longer than 100 Unicode code points. Invalid queries are rejected before Redis or TMDB is accessed.
  ```json
  [
    {
      "id": 550,
...
* **Method:** `GET`
* **Access:** 受保護 (Private)
* **Query Parameters:** `query` (字串，必填，1–100 個 Unicode 字元，依碼位計算)
* **成功回應:** `200 OK` (若 Redis 可用，將優先使用快取資料)
* **錯誤回應:** 查詢缺少、空白或超過 100 個 Unicode 字元（依碼位計算）時回傳 `400 Bad Request`。無效查詢會在存取 Redis 或 TMDB 前遭到拒絕。
  ```json
  [
    {
      "id": 550,
